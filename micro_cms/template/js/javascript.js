    ...
    (function () {
        ...
        function checkEmail(str) {
            ...
            if (pattern.test(str) == false) {
                $get("email_pattern").innerText = "введите правильно email";
                bool = false;
            }
            else {
        ...
        function checkPassword() {
            if (pass.value.toString() !== repeat.value.toString()) {
                bool = false;
                $get('pass_equal').innerText = "пароли не совпадают";
            }
            else {
        ...
        login.onblur = function () {
            ...
            xhr.onreadystatechange = function () {
                if (xhr.status == 200 && xhr.readyState == 4) {
                    if (xhr.responseText == '1') {
                        $get("login_used").innerText = "Login " + login.value + " уже используется";
                        bool_user = false;
                    }
                    else {
            ...
            xhr.open('get', '/core/login_return.php?login=' + login.value, false);
            xhr.send();
        }
    }).call(this);
};